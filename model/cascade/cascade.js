...
module.exports = {
  ...
  //http://localhost:3000/testcase/queryById?ID=2
	...
	//http://localhost:3000/select/getTestRelease
	getTestRelease : function(req, res, next) {
		var json = '';
		pool.getConnection(function(err, connection) {
			if (err)
				throw err;
			console.log("-----------------------");
			console.log($sql.getTestRelease);
			connection.query($sql.getTestRelease, function(err, result) {
				res.json(result);
				connection.release();
			});
		});
	},
	getTestBuildByRelease : function(req, res, next){

		var release = req.query.releaseNumber;
		pool.getConnection(function (err, connection){
			console.log("-----------------------");
			console.log($sql.getTestBuildByRelease);
			console.log("release number is:" + release );
			connection.query($sql.getTestBuildByRelease, release, function(err, result) {
				res.json(result);
				console.log(result);
				connection.release();
	...
	getrunTestByBuild: function(req, res, next){

		var testBuildNumber = req.query.testBuildNumber;
		pool.getConnection(function (err, connection){
			console.log("-----------------------");
			console.log($sql.getrunTestByBuild);
			console.log("testBuild number is:" + testBuildNumber );
			connection.query($sql.getrunTestByBuild, testBuildNumber, function(err, result) {
				res.json(result);
				console.log(result);
				connection.release();
